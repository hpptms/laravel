...
/*!*************************************!*\
  !*** ./resources/js/event-crate.js ***!
  \*************************************/
var number = 2;
$('#tag-add').click(function () {
  if (number <= 5) {
    $('#tags').append("<input type='tag' class='form-control' id='tag' name='tag".concat(number, "'>"));
    number = ++number;
  }
});
...
;
var photo = $('#load_photo').text();

if (photo != '') {
  $('#Preview').css('background-image', "url(".concat(photo, ")"));
  $('#div_Preview').show();
  $('.photo-content').animate({
    "height": "624px"
  }, "slow");
...
var marker;
var lat = jQuery('#load_lat').text();

var _long = jQuery('#load_long').text();
...
var geocode_to_latlond = function geocode_to_latlond() {
  var geocoder = new google.maps.Geocoder();
